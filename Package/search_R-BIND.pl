...
my @database=`cat $script_dir/R-BIND_site2motif.txt`;chomp(@database);
...
foreach my $line(@id_motif)
{
	if($line=~/^(\S+)\s+loop:\s+(.+)/)
	{
		...
		$num++;
		my @rbind_num=();
		open(OUT2,">$output_dir/$query_id\_r$num.out");
		foreach my $data(@database)
		{
			...
			if($data=~/^($type\d+\s+.+)\s+(\S+)/)
			{
				...
				$result_str .=&motif_align($script_dir,$line,$motif_db,$ligand,$rbind_num[-1]);
			}
		}
...
open(OUT1,">$output_dir/$query_id\_rfinal.out");
for(my $i=1;$i<$num+1;$i++)
{
	if(-s "$output_dir/$query_id\_r$i.out")
	{
		my $result_file="$output_dir/$query_id\_r$i.out";
